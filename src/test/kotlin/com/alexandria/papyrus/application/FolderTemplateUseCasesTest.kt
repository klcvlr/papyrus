...
class FolderTemplateUseCasesTest {
    ...
    fun `a folder template can be created`() {
        ...
        val folderTemplateIdentifier = folderTemplateUseCases.create("folderTemplateName")
    ...
    fun `a folder template can be renamed`() {
        ...
        folderTemplateUseCases.rename("folderTemplateIdentifier", "newFolderTemplateName")
    ...
    fun `a sub folder template can be added to a folder template`() {
        val folderTemplate = aFolderTemplate(identifier = "folderTemplateIdentifier")
        every { folderTemplateRepository.findByIdentifier("folderTemplateIdentifier") } returns folderTemplate
        every { folderTemplateAndFolderService.addSubFolderTemplate(folderTemplate, "subFolderTemplateName") } returns "subFolderTemplateIdentifier"
        every { folderTemplateRepository.save(any()) } returns Unit

        val subFolderTemplateIdentifier = folderTemplateUseCases.addSubFolder("folderTemplateIdentifier", "subFolderTemplateName")
    ...
    fun `a folder template's associated document type can be changed`() {
        every { folderTemplateAndFolderService.changeAssociatedDocumentType("folderTemplateIdentifier", "documentTypeIdentifier") } returns Unit

        folderTemplateUseCases.changeAssociatedDocumentType("folderTemplateIdentifier", "documentTypeIdentifier")

        verify(
            exactly = 1,
        ) { folderTemplateAndFolderService.changeAssociatedDocumentType("folderTemplateIdentifier", "documentTypeIdentifier") }
    }
}
